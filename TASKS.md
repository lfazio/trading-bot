...
- **CR-007 — Monte Carlo simulation in backtest** (Accepted 2026-05-16 @ wiki 283041f; design cascade pending). Scope: Python-only v1; `MCGenerator` Protocol with block-bootstrap + GBM + regime-stitched generators; bit-identical replay under `(strategy, seed, generator_config, n_paths)`; v1 archives summary percentiles only, not per-path histories; composes the existing deterministic `backtesting/engine.py` with no refactor. Reserves `REQ_F_MCS_001..006`, `REQ_NF_MCS_001`, `REQ_SDS_MCS_*`, `REQ_SDD_MCS_*`, `REQ_TP_MCS_*` per REQ_NF_LIF_001. Sequenced *before* CR-012.
