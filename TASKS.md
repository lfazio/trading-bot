...
- [~] `institutional/` — phase-6 features: vol-target sizer, risk-parity allocator, strategy ensemble, hedge-overlay manager — partial 2026-05-08: **vol-target sizer** + **risk-parity allocator** + **strategy ensemble** all live in `strategies/ensemble.py` (REQ_F_STR_004 / REQ_SDD_ALG_010 at TEST). **NAV / attribution reporting** lives in `portfolio.attribution()` + `analytics.Analytics` (REQ_F_PRT_002 at TEST). **Hedge-overlay manager** filed as **CR-012** (Accepted 2026-05-16 @ wiki 283041f; design cascade pending — `REQ_F_HOV_001..005`, `REQ_NF_HOV_001`, `REQ_SDS_HOV_*`, `REQ_SDD_HOV_*`, `REQ_TP_HOV_*` reserved per REQ_NF_LIF_001; sequenced after CR-007 so overlay sizing can consume MC shock distributions; phase-6 no-op gating mandatory; reuses CR-011's append-only hedge-ledger pattern).
