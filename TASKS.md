...
- [~] `regime/` — Market-regime detector + transition tracker (CR-013, Phase 5) — design cascade complete 2026-05-15 (4 wiki commits: SRS @ 92f909f, SDS @ 752f691, SDD @ c3a80c6, TP @ bcba672). Foundation slice in (this commit): `RegimeDetector.evaluate` with the documented `RULE_ORDER` constant + MA-crossover + vol-band rule; `TransitionTracker` with single mutable cursor + confirmation window + flip-back reset + `from_seed` restart hook; `RegimeConfig` with all invariants enforced at construction; `BarSource` Protocol skeleton; 30 tests covering TC_RGM_001..007 + TC_RGM_010. 10 RGM REQs at TEST; 4 still at TP / CODE — pending follow-up: CR-008 `TransitionRepository` slot + `0002_regime.sql` migration (REQ_SDD_RGM_005); SafetyLayer wiring of the confirmed transition into `SafetyLayer.raise_trigger` (REQ_F_RGM_005 / REQ_SDD_RGM_004); consumer wiring to sector_rotator / structured_products / risk-engine regime gate (REQ_SDS_RGM_001).
