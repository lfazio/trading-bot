...
- [~] **C3 / C9..C14** — Phase-8 Part C hardening; partial
      ...
      - **C3** Pydantic v2 schemas across 8 of 10 typed-loader
        YAMLs ✅ DONE 2026-06-01 @ `<this commit>` (three-slice
        delivery). New module
        ...
        sub-trees with full Decimal invariant ranges),
        `mc_drawdown_floor.yaml` (CR-031 matrix + default
        with Phase-enum-name + MarketRegime-value
        membership), `system.yaml` (starting_capital +
        broker + data provider; hex-seed string coercion),
        `turbos.yaml` (filter + scoring with 4-weight
        sum-to-one invariant), `webui.yaml`
        (host/port/idempotency_backend), and `logging.yaml`
        (level + format + nullable file_path). `RICH_SCHEMAS`
        table drives the validator; remaining YAMLs
        (`phases.yaml`, `quant.yaml`, `accounts.yaml`) have
        complex nested shapes and land in their own focused
        follow-up slice.
        ...
        in to the tree-shaped output. 44 schema tests + 2 CLI
        ...
        method on the top-level model; Decimal coercion via
