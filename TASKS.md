...
- [~] `institutional/` — phase-6 features: vol-target sizer, risk-parity allocator, strategy ensemble, hedge-overlay manager — partial 2026-05-08: **vol-target sizer** + **risk-parity allocator** + **strategy ensemble** all live in `strategies/ensemble.py` (REQ_F_STR_004 / REQ_SDD_ALG_010 at TEST). **NAV / attribution reporting** lives in `portfolio.attribution()` + `analytics.Analytics` (REQ_F_PRT_002 at TEST). **Hedge-overlay manager** filed as **CR-012** — design cascade complete 2026-05-16 (4 wiki commits: SRS @ 4db591e, SDS @ 7b4f2d9, SDD @ 3e901db, TP @ cd6cda6). 12 REQs at TP: REQ_F_HOV_001..005, REQ_NF_HOV_001, REQ_SDS_HOV_001..002, REQ_SDD_HOV_001..004. v1: index futures only (linear delta hedge — option overlays a follow-up CR); EUR-denominated overlays only (CR-011 coordination deferred); EuroSTOXX 50 benchmark; 60-day rolling beta matching REQ_F_RSK_003. `OverlayPolicy` defaults pinned (`target_beta=0.5`, `target_vol=0.12`, `beta_band=0.05`, `max_overlay_pct=0.10` hard ceiling per REQ_F_CAP_011); `HedgeOverlay.size` checks phase gate first (sub-Phase-6 returns `Ok(())` before reading other inputs); reuses CR-011's append-only ledger pattern (NO `InstrumentClass.OVERLAY` extension); deterministic mark `notional × (current/entry - 1)` + carry `notional × pct × days/365`; tax accessors per REQ_C_TAX_001 (gain × 0.70; losses pass through). CR-007 → CR-012 follow-up sub-CR (overlay sizer consumes MC shock distributions) opens once both implementations ship.
- [ ] `backtesting/monte_carlo/` — Monte Carlo simulation (CR-007, Phase 6/7) — design cascade complete 2026-05-16 (4 wiki commits: SRS @ e3564d1, SDS @ 2b84446, SDD @ 958e63c, TP @ 71a821d). 17 REQs at TP: REQ_F_MCS_001..006, REQ_NF_MCS_001, REQ_SDS_MCS_001..004, REQ_SDD_MCS_001..006. Composes the existing deterministic `backtesting/engine.py` without modification — runner injects a `backtest_factory` callable; per-path RNG seeded via `sha256(seed||path_index)[:8]` so two parallel callers with the same `(seed, path_index)` see identical bytes. Closed v1 generator set: BlockBootstrap (i.i.d. block resampling) / GBM (one-step Normal transition) / RegimeStitched (per-regime sub-blocks via CR-013 detector). v1 archives summary percentiles only — closed quintile keyset `{0.05, 0.25, 0.50, 0.75, 0.95}` with monotonicity invariant + KS trip rate + `config_hash` for CR-008 join. Meta-loop integration via `strategy_lab/loop_controller.py` rejects candidates whose 5th-percentile drawdown exceeds the phase floor. Sequenced *before* CR-012 (hedge overlay sizing consumes MC shock distributions).
