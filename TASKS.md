...
- [~] `wealth_ops/` — phase-5 features: tax-loss harvester, sector rotator, currency hedger — partial 2026-05-08: **tax-loss harvester** shipped under `tax/harvest.py` (REQ_F_TAX_006 at TEST). **Sector rotation** and **currency hedging** have no SRS REQs — filed as **CR-010** and **CR-011** (Proposed); implementation begins on lifecycle cascade after operator acceptance. The `wealth_ops/` package itself is created on cascade entry, not before.
