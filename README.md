...
|---|---|---|
| 1 — Software Requirements (SRS) | [Wiki › SRS](https://github.com/lfazio/trading-bot/wiki/SRS) | Approved at `7424909`; re-approved 18× through 2026-05-23 (CR cascades) |
...
| 5 — Implementation | `trading_system/` | **All Phase-5 + Phase-6 lifecycle work complete.** Modules shipped: persistence (CR-008), regime detector (CR-013), fundamentals provider (CR-014), trade rationale audit trail (CR-015), currency hedger (CR-011), operator-grade webapp step 1 (CR-019), Plotly + Kaleido reports (CR-020), range-aware cache (CR-021), live-bypass paper polling (CR-022); 2 684 tests green |
| 6 — Test Execution | `tests/` | Continuous — 2 684 passing |
...
| 8 — Operator Hardening Sprint | `Documentations/Feature-Gap-Analysis-2026-05-23.md` Part C | 6 of 8 items done; 2 deferred (C4 filed as CR-024 Proposed; C7 dynamic needs Docker daemon) |
...
live-trading mode (CR-019 step 2) is gated on broker-adapter
selection per `REQ_F_BRK_003`.
...
### Change-request cascades complete (2026-05-22 → 2026-05-23)

Post-Phase-7 wave delivering the operator-grade surface:

- **CR-019 (operator-grade webapp, step 1)** — ✅ Done (2026-05-23).
  Paper-trading runtime + onboarding wizard + dashboard panels +
  backtest workflow + reports panel + strategy registry +
  recovery wizard + multi-account switcher + accessibility
  audits. 34 new REQs at TEST.
- **CR-020 (Plotly + Kaleido replace matplotlib)** — ✅ Done.
  `equity-curve.html` is now self-contained interactive Plotly
  (JS bundle inlined; no CDN; no-Node invariant preserved); PNG
  via Kaleido. Compare-two-runs helper for CR-019's backtest
  panel.
- **CR-021 (range-aware YFinanceCache lookup)** — ✅ Done.
  Cache hit now satisfied by either exact-key match OR envelope
  match; sliced read byte-equal to an exact-key recorder run.
  ~20× speedup on the CAC 40 / 17-month default backtest.
- **CR-022 (live-bypass fetch for paper-trading polls)** — ✅ Done.
  After backfill, the paper bar source force-refreshes upstream
  per poll (bypassing the CR-021 envelope cache); falls back to
...
- **C5 — Persistence migration drill** — ✅ Done. WAL recovery
  ...
  boundary audit. Strengthens REQ_F_PER_001/003/004/010,
